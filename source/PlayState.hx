...
class PlayState extends MusicBeatState
{
	...
	public var lightBar:FlxSprite;
	public var lightDarkness:FlxSprite;
	public var lightsNumber:Int = 0;
	...
	override public function create()
	{
		...
		lightBar = new FlxSprite(healthBarBG.x - 50, -75).loadGraphic(Paths.image('light-bar'));
		lightBar.scrollFactor.set();
		lightBar.visible = !ClientPrefs.hideHud;
		lightBar.antialiasing = ClientPrefs.globalAntialiasing;
		lightBar.scale.set(0.5, 0.5);
		lightBar.screenCenter(X);
		add(lightBar);
		...
		lightDarkness = new FlxSprite();
		lightDarkness.frames = Paths.getSparrowAtlas('lights/lightdark');
		lightDarkness.animation.addByPrefix('light1', 'light1', 24);
		lightDarkness.animation.addByPrefix('light2', 'light2', 24);
		lightDarkness.animation.addByPrefix('light3', 'light3', 24);
		lightDarkness.animation.addByPrefix('light4', 'light4', 24);
		lightDarkness.animation.addByPrefix('dark-1', 'dark-1', 24);
		lightDarkness.animation.addByPrefix('dark-2', 'dark-2', 24);
		lightDarkness.animation.addByPrefix('dark-3', 'dark-3', 24);
		lightDarkness.animation.addByPrefix('dark-4', 'dark-4', 24);
		lightDarkness.animation.addByPrefix('nothing0', 'nothing', 24);
		lightDarkness.animation.play('nothing0');
		lightDarkness.scrollFactor.set();
		lightDarkness.screenCenter(XY);
		lightDarkness.scale.set(1.7, 1.7);
		...
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if(note.noteType == 'Light Note') {
					...
					if(lightsNumber < 0) {
						lightAnimation = 'dark';
					} else if(lightsNumber > 0) {
						lightAnimation = 'light';
					} else if(lightsNumber == 0) {
						lightAnimation = 'nothing';
					}
					lightDarkness.animation.play(lightAnimation + lightsNumber);
				}

				if(note.noteType == 'Dark Note') {
					if(lightsNumber != -4) {
						lightsNumber--;
